...
```{r hist, fig.height=4, tidy=FALSE}
...
```
  ...
  Indeed, mean through summarize is: `r mean_sum` and median is `r median_sum`.
...
```{r step_interval, warning=FALSE,fig.height=4}
...
``` {r max_number_steps,  results="hide"}
...
The mean total number of steps taken per day is **`r mean_totalStepsPerDay`**  
The median total number of steps taken per day is **`r median_totalStepsPerDay`**
...
``` {r lastplot, warning=FALSE,fig.height=10, tidy=FALSE}
library(lattice)
